...
# ----------------------------------------------------------
st.set_page_config(
    page_title="NEON GRID CONTROL v26.1", 
    layout="wide", 
    page_icon="⚡",
...
def calculate_voltage_profile(bus_name, p_kw, q_kvar, tap_pos):
    dist = dist_map.get(bus_name, 1.0)
    if dist < 1e-6: dist = 1e-6 
    z_line = LINE_IMPEDANCE_PER_UNIT_DIST * dist
    v_source = 1.0 * tap_pos
    s_apparent = complex(p_kw, q_kvar) / 1000.0
    i_approx = s_apparent.conjugate() / v_source
...
# ----------------------------------------------------------
# NEW HELPER: SOLAR & DISPATCH LOGIC
# ----------------------------------------------------------
def get_solar_contribution(hour_idx):
    """
    Returns the target Solar % and Grid % based on Hour of Day.
    Logic:
      - 7 PM - 6 AM: Night (0% Solar)
      - 6 AM - 10 AM: Ramp Up (0 -> 60%)
      - 10 AM - 3 PM: Peak (80% Solar)
      - 3 PM - 7 PM: Ramp Down (60% -> 0%)
    """
    h = hour_idx % 24
    
    if h < 6 or h >= 19: # Night
        return 0.0
    elif 6 <= h < 10: # Ramp Up
        # Map 6->0.15, 7->0.30, 8->0.45, 9->0.60
        return 0.15 * (h - 5)
    elif 10 <= h < 15: # Peak (Master)
        return 0.80
    elif 15 <= h < 19: # Ramp Down
        # Map 15->0.60, 16->0.45, 17->0.30, 18->0.15
        return 0.60 - (0.15 * (h - 15))
    return 0.0
...
def render_home():
    ...
    # ----------------------------------------
    # SOLAR DISPATCH LOGIC (DAY/NIGHT/SEASON)
    # ----------------------------------------
    sim_hour = idx % 24
    sim_month = (idx // 720) % 12
    season_factor = 1.0 if (4 <= sim_month <= 9) else 0.6
    base_solar_pct = get_solar_contribution(sim_hour)
    solar_pct = base_solar_pct * season_factor
    
    p_total = row["Total_Active_Power"] + st.session_state.hvac_load_kw
    p_solar = p_total * solar_pct
    p_grid = p_total * (1 - solar_pct)
    q_val = row["Total_Reac_Power"] + (st.session_state.hvac_load_kw * 0.6)
    
    recloser_logic()
    freq, temp = update_grid_physics(p_grid, q_val)
    
    delta_p = p_total - st.session_state.prev_p
    delta_q = q_val - st.session_state.prev_q
    st.session_state.prev_p = p_total
    ...
    c1, c2 = st.columns(2)
    with c1:
        st.plotly_chart(make_cyber_meter(p_total, delta_p, "TOTAL ACTIVE POWER (kW)", 0, max_p, "#00f3ff"), use_container_width=True, key="gauge_p")
        hist_p = df_raw["Total_Active_Power"].iloc[max(0, idx-60):idx]
        st.plotly_chart(make_cyber_plot(hist_p.index, hist_p, "LOAD HISTORY", "#00f3ff", delta_val=delta_p, height=200), use_container_width=True, key="trend_p")
    with c2:
        st.plotly_chart(make_cyber_meter(q_val, delta_q, "REACTIVE POWER (kVAR)", 0, max_q, "#ff00ff"), use_container_width=True, key="gauge_q")
        hist_q = df_raw["Total_Reac_Power"].iloc[max(0, idx-60):idx]
        st.plotly_chart(make_cyber_plot(hist_q.index, hist_q, "VAR HISTORY", "#ff00ff", delta_val=delta_q, height=200), use_container_width=True, key="trend_q")
...
def render_feeder(view_bus):
    ...
    # 1. Physics Engine: Calculate True Voltage
    voltage_pu_phys = calculate_voltage_profile(view_bus, display_p, display_q, st.session_state.tap_position)
    ...
    measured_p = apply_scada_noise(display_p, 1.0)
    measured_q = apply_scada_noise(display_q, 1.0)
    ...
    # --- NEW: Update Residual Buffer ---
    st.session_state.history_se_j.append(se_chi)
    if len(st.session_state.history_se_j) > 50: st.session_state.history_se_j.pop(0)

    pf_denom = np.sqrt(display_p**2 + (display_q - st.session_state.capacitor_bank_kvAr)**2)
    pf_final = display_p / pf_denom if pf_denom > 0 else 1.0
    ...
    m2.metric("LOAD", f"{display_p:.1f} kW", delta=f"{delta_feeder:.1f} kW")
    
    # CYBER ATTACK ALERT ON SE METRIC
    ...
    with st.expander("🧠 STATE ESTIMATION (WLS) ENGINE & SECURITY", expanded=True):
        ...
        # VISUAL: WLS Noise Filtering Plot
        fig_se_plot = go.Figure()
        x_axis = list(range(len(st.session_state.history_se_meas)))
        
        fig_se_plot.add_trace(go.Scatter(
            x=x_axis, y=st.session_state.history_se_meas, 
            mode='markers+lines', name='SCADA (Raw/Bad)', 
            line=dict(color='#ff0055', width=1, dash='dot'),
            marker=dict(size=4)
        ))
        
        fig_se_plot.add_trace(go.Scatter(
            x=x_axis, y=st.session_state.history_se_est, 
            mode='lines', name='Estimated (WLS)', 
            line=dict(color='#00ff00', width=3)
        ))
        
        fig_se_plot.update_layout(
            height=250, margin=dict(l=20, r=20, t=30, b=20),
            title=dict(text="REAL-TIME STATE ESTIMATOR CONVERGENCE", font=dict(size=12, color="white", family="Orbitron")),
        ...
        # --- NEW: WLS COST FUNCTION J(x) PLOT ---
        fig_j = make_cyber_plot(
            list(range(len(st.session_state.history_se_j))),
            st.session_state.history_se_j,
            "SE RESIDUAL COST J(x) - (BAD DATA DETECTOR)",
            "#ffae00", # Warning Orange
            height=150
        )
    ...
    with col_tap:
        with st.container(border=True):
            ...
            tap_fig = make_cyber_plot(list(range(len(st.session_state.history_tap))), 
                                      st.session_state.history_tap, 
                                      "TAP CHANGE HISTORY", "#ffae00", height=100)
    ...
    with st.expander("📉 HARMONICS (IEEE 519)", expanded=False):
        ...
        with pq_col2:
            ...
            fig_wave.update_layout(
                title="OSCILLOSCOPE", xaxis_title="ms", yaxis_title="V",
                height=250, margin=dict(l=20, r=20, t=30, b=20),
                paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0.3)",
                font=dict(color="#ccc", family="Orbitron"), xaxis=dict(gridcolor="#333"), yaxis=dict(gridcolor="#333")
            )
    ...
    with st.expander("⚡ APFC CONTROL (Capacitor Switching)", expanded=True):
        ...
        cap_fig = make_cyber_plot(list(range(len(st.session_state.history_cap))), 
                                  st.session_state.history_cap, 
                                  "kVAR INJECTED", "#00ff00", height=150)
...
def get_node_sim_data(bus_name, current_idx):
    ...
    # Use Physics Engine for Voltage
    v_pu = calculate_voltage_profile(bus_name, p_kw, q_kvar, st.session_state.tap_position)
    
    i_amps = (np.sqrt(p_kw**2 + q_kvar**2) / (0.208 * 1.732)) if bus_name not in TRANSFORMER_NODES else 0.0
    pf = p_kw / np.sqrt(p_kw**2 + q_kvar**2) if p_kw > 0 else 1.0
    return v_pu, i_amps, p_kw, q_kvar, pf
...
def render_topology():
    ...
    for bus, (bx, by) in bus_dict.items():
        ...
        if bus in TRANSFORMER_NODES: n_type, col, size, sym = "TRANSFORMER", "#ffae00", 12, "star"
        elif "source" in bus or bus == "bus1": n_type, col, size, sym = "SUBSTATION", "#ffffff", 18, "diamond"
        else: n_type, col, size, sym = "LOAD BUS", "#00f3ff", 6, "circle"
    ...
    with col_details:
        if selected_node != "All":
            v, i, p, q, pf = get_node_sim_data(selected_node, st.session_state.idx)
            st.markdown(f"""
